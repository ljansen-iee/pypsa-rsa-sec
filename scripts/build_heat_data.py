...
from _helpers import remove_leap_day, set_year_of_datetime_index
...
def generate_periodic_profiles(dt_index, nodes, weekly_profile, localize=None):
    ...
    for node in nodes:
        timezone = pytz.timezone(pytz.country_timezones['ZA'][0])
...
def prepare_heat_data(n, year):
    
    ashp_cop = (
        xr.open_dataarray(snakemake.input.cop_air_total)
        .to_pandas()        
        .pipe(remove_leap_day)
        .pipe(set_year_of_datetime_index, year)
    ...
    energy_totals = pd.read_csv(
        ...
        index_col=[0,1], #energy totals is multi-year
...
if __name__ == "__main__":
    ...
    (
        ...
        solar_thermal,
        district_heat_share,
    ) = prepare_heat_data(n, investment_year)
